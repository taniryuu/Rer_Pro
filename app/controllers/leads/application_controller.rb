class Leads::ApplicationController < Users::ApplicationController
  ...
  def destroy_step(lead, step)
    ...
    if lead.errors.blank?
      ...
      Task.create!(step_id: step.id ,name: name, status: "not_yet", scheduled_complete_date: scheduled_complete_date)
      redirect_to step
    end
  end
  ...
  def check_status_and_redirect_to(step, redirect_to_step, roop_ok)
    if (roop_ok.present? && roop_ok == "true") || ($through_check_status == false)
